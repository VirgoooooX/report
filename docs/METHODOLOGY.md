...
本项目是数据密集型 Dashboard，页面级滚动、表格横向滚动、矩阵/时间线内部滚动会同时存在。滚动条必须作为设计系统的一部分统一管理，不能由各组件临时覆盖。

**统一规则**：
- 页面主滚动区域保留稳定 gutter，避免滚动条出现/消失导致整体宽度变化。
- 全局滚动条样式以 `frontend/src/assets/styles/variables.css` 和 `global.css` 为准；组件中新增滚动容器时，应复用全局规则或公共工具类。
- 内部滚动容器（如 Test Summary、Schedule Timeline、Heatmap、Query Center 表格）使用薄滚动条和透明轨道，不应挤压布局、不应改变卡片/表格尺寸。
- 深色模式下滚动条颜色必须从 Design Token 或全局变量派生，不允许硬编码一套浅色样式。
- 如确实需要 hover 才显示滚动条，必须统一实现 WebKit 与 Firefox 行为，并保证移动端可滚动、桌面端不抖动。

**开发检查**：
- 搜索新增的 `overflow: auto`、`overflow-x: auto`、`overflow-y: auto`，确认对应容器使用统一滚动条样式。
- 在桌面宽屏、窄屏和深色模式下检查滚动条是否造成内容跳动。
- 表格、时间线、热力图这类固定表头/固定列组件，要同时验证 sticky 区域与滚动条之间没有遮挡或错位。
